...
# courses = make_api_call('GET', f'/cr/professor/{prof_id}/courses')
...
# # See topics students are working on [Professor-1.1]
# st.subheader("📖 Topics Students are Working On")

# topics_students = make_api_call('GET', f'/cr/course/{selected_course["CRN"]}/topics/students')

# if topics_students:
#     df_topics = pd.DataFrame(topics_students)
    
#     if not df_topics.empty and 'topicName' in df_topics.columns:
#         # Show topic distribution
#         topic_counts = df_topics.groupby('topicName').size().reset_index(name='student_count')
        
#         fig_topics = px.bar(
#             topic_counts,
#             x='topicName',
#             y='student_count',
#             title=f'Number of Students per Topic in {selected_course.get("name", "Course")}',
#             labels={'topicName': 'Topic', 'student_count': 'Number of Students'},
#             color='student_count',
#             color_continuous_scale='blues'
#         )
#         st.plotly_chart(fig_topics, use_container_width=True)
...
#     else:
#         st.info("No student activity on topics yet for this course")
# else:
#     st.info("No topic data available")

# st.write("---")

# # Study time analytics [Professor-1.6]
# st.subheader("⏱️ Study Time by Topic")

# study_time_data = make_api_call('GET', f'/si/professor/{prof_id}/study_time/topics')
...
#         with col1:
#             total_minutes = df_study['Total_Study_Time'].sum()
#             total_hours = total_minutes / 60
#             st.metric("Total Study Time", f"{total_hours:.1f} hours")
        
#         with col2:
#             avg_minutes = df_study['Avg_Minutes_Per_Session'].mean()
#             st.metric("Avg Session Length", f"{avg_minutes:.1f} minutes")
        
#         with col3:
#             total_sessions = df_study['Session_Count'].sum()
...
#         # Bar chart
#         fig = px.bar(
#             df_study.sort_values('Total_Study_Time', ascending=True).tail(10), 
#             x='Total_Study_Time', 
#             y='Topic',
#             orientation='h',
#             title=f"Top 10 Topics by Total Study Time",
#             labels={'Total_Study_Time': 'Total Study Time (minutes)', 'Topic': 'Topic'},
#             color='Total_Study_Time',
...
#             if selected_topic:
#                 sessions = make_api_call('GET', f'/si/sessions/topic/{selected_topic}')
                
#                 if sessions and len(sessions) > 0:
#                     st.write(f"**Sessions covering '{selected_topic}':**")
                    
#                     df_sessions = pd.DataFrame(sessions)
#                     st.dataframe(df_sessions, use_container_width=True, hide_index=True)
                    
#                     # Show individual session details
#                     if not df_sessions.empty and 'sessionID' in df_sessions.columns:
#                         session_id = st.selectbox(
#                             "Select session for detailed view:",
#                             df_sessions['sessionID'].tolist()
#                         )
                        
#                         if st.button("Get Session Details"):
#                             session_details = make_api_call('GET', f'/si/study_session/{session_id}')
...
#                                     st.json(session_details)
#                 else:
#                     st.info(f"No sessions found for topic: {selected_topic}")
#     else:
#         st.info("No study time data available yet")
# else:
#     st.info("No study session data available for your courses")
