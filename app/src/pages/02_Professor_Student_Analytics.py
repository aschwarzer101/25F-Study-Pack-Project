...
topics_response = requests.get(f'http://api:4000/s/study_time/professor/{st.session_state['professor_id']}/topics')
...
sessions = requests.get(f'http://api:4000/s/sessions/topic/{selected_topic}').json()

for session in sessions:
    with st.expander(f"Session {session['sessionID']} - {session['date']}"):
            session_details = requests.get(f'http://api:4000/s/study_session/{session["sessionID"]}').json()
